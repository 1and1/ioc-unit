...
public class SimulatedTransactionManager {
    ...
    /**
     * Start next transactioncontext of this thread by pushing the {@link TxType} on the stack.
     *
     * @param Transactional.TxType The Transactionattribute to be stacked.
     */
    public void push(TxType txType) {
        ArrayList<ThreadLocalTransactionInformation> stack = initStack();
        stack.add(new ThreadLocalTransactionInformation(txType));
    }
    ...
    public void takePart(PersistenceFactory persistenceFactory) {
        ...
        for (int i = pos + 1; i < stack.size(); i++) {
            ...
            TxType attribute = threadLocalTransactionInformation.txType;
    ...
    static class ThreadLocalTransactionInformation {
        ...
        ThreadLocalTransactionInformation(TxType txType) {
            this.txType = txType;
            userTransaction = false;
        }

        ThreadLocalTransactionInformation() {
            this.txType = TxType.REQUIRES_NEW;
        ...
        void setRollbackOnly() {
            rollbackOnly = true;
            if(txType == TxType.REQUIRED && previous != null) {
                if(previous.txType == TxType.REQUIRED || previous.txType == TxType.REQUIRES_NEW) {
        ...
        public void setRolledBack() {
            rolledBack = true;
            if(previous != null) {
                if(txType == TxType.REQUIRED
                   || txType == TxType.SUPPORTS
                   || txType == TxType.MANDATORY) {
                    if(previous.txType != TxType.NOT_SUPPORTED) {
        ...
        public int getStatus() {
            if(isUserTransaction() || txType == TxType.REQUIRED
               || txType == TxType.REQUIRES_NEW
               || txType == TxType.MANDATORY) {
                if(getRollbackOnly()) {
            ...
            else if(txType == TxType.SUPPORTS) {
